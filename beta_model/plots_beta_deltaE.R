...
library(stringr)
setwd('D:/LuoAo/LP/data/')
# DATA1 -------------------------------------------------------------------
dat <- data.frame()

fls <- list.files('ELEV/')[1:28]
elevs <- list()
for (fl in fls) {
  tmp <- raster(paste0('ELEV/',fl,'/',fl,'_dem.tif')) 
  elevs <- c(elevs,list(tmp))
}
names(elevs) <- fls

cums <- read.csv('mountain/data1_MIREN/MIREN_plant_records_data2007-2015.lat.long.csv')
lat <- (cums$lat%/%1)
...
# DATA2 -------------------------------------------------------------------
sp <- read.csv('data2_doi_10_5061_dryad_rbnzs7hdt__v20220815/bunya_species_list.csv')
sp <- sp[,1:3]
cums <- read.csv('data2_doi_10_5061_dryad_rbnzs7hdt__v20220815/alison_low_east_and_low_west_plots.csv')
cums <- cums %>% left_join(sp)
cums <- cums[,c('location','plot','current_taxon')]

tmp <- read.csv('data2_doi_10_5061_dryad_rbnzs7hdt__v20220815/butler_high_east.csv')
...
tmp <- read.csv('data2_doi_10_5061_dryad_rbnzs7hdt__v20220815/butler_mid_east.csv')
tmp <- tmp[,c('abbrev','x','y')]
tmp$plot <-  0
flag <- (tmp$x<=25)&(tmp$y<=25) ; tmp$plot[flag] <- 1
flag <- (tmp$x<=25)&(tmp$y>=75) ; tmp$plot[flag] <- 2
...
tmp <- read.csv('data2_doi_10_5061_dryad_rbnzs7hdt__v20220815/butler_mid_west.csv')
tmp <- tmp[,c('abbrev','x','y')]
tmp$plot <-  0
flag <- (tmp$x<=25)&(tmp$y<=25) ; tmp$plot[flag] <- 1
flag <- (tmp$x<=25)&(tmp$y>=75) ; tmp$plot[flag] <- 2
...
# DATA3 -------------------------------------------------------------------
cums <- read.csv('data3_doi_10_5061_dryad_8gr16__v20181004/Nabe-Nielsen etal 2017 Young Sund spp pres in plotgrps.csv')
...
# DATA4 -------------------------------------------------------------------
cums <- read.csv('data4/PFTC3-Puna-PFTC5_Peru_2018-2020_CommunityCover_clean.csv')
...
write.csv(dat,'beta_deltaE.csv',row.names = F)
