...
mycolor <- c('#803429','#F1B972','#F3E887','#8BCBEF','#8BCBEF','#CAD1DB')

dat$gmm_GI
p <- func('gmm_GI','active',dat);p+xlab('Gamme_GI')

flag <- which(!is.na(dat$active))
dat$active <- dat$active.ori
dat$active[flag] <- dat$active[sample(flag)]
p2 <- func('gmm_GI','active',dat);p2+xlab('Gamme_GI')