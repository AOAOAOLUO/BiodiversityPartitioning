...
for (gmm in levels(pdat$gamma_anomaly)) {
  for (act in levels(pdat$active)){
    ...
    x <- pdat.tmp$gamma_anomaly_p[!pdat.tmp$obs]
    #x <- mu-x
    mu <- pdat.tmp$gamma_anomaly_p[pdat.tmp$obs]
    #t.res <- t.test(x, mu = 0)
    t.res <- t.test(x, mu = mu)
    p.value <- t.res$p.value
    print(p.value)
    t.mark <- ifelse(p.value>0.05,'ns','*')
    t.mark <- paste0(t.mark,ifelse(p.value>0.001,'','*')) 
    t.mark <- paste0(t.mark,ifelse(p.value>0.0001,'','*')) 
    
    tmp <- data.frame(gmm=gmm,
                      active=act,
                      obs=mu,
                      null.mean=mean(x),
                      null.low=mean(x)-sd(x),
                      null.high=mean(x)+sd(x),
                      p=p.value,
                      t.mark=t.mark)
    pdat.s <- rbind(pdat.s,tmp)
  }
}

pdat.s$gmm <- factor(pdat.s$gmm,c('Top 1%','Top 5%',
                                  'Top 10%','Top 25%','Top 50%','Low') %>% rev())
pdat.s$grp <- 'OBS'
pdat.s.2 <- pdat.s
pdat.s.2$active <- factor(pdat.s.2$active,levels = c(
  "Active","Geomorphic active","Compression","Compressive shear",
  "Extensional shear","Extension"))
#pdat.s.2$active.num <- as.numeric(pdat.s.2$active)
tmp <- pdat.s.2[pdat.s.2$active%in%'Active',]
levels(tmp$gmm) <- stringr::str_remove(levels(tmp$gmm),'Top ')
tmp <- tmp[,c('gmm',"null.mean","null.low","null.high")]
colnames(tmp)[1] <- 'gamma_anomaly'

p.geo.active <- left_join(p.geo.active,tmp)

mycolor <- c('#803429','#F1B972','#F3E887','#8BCBEF','#0156FF','#CAD1DB') %>% rev()
mycolor.light <- lapply(mycolor, function(x)colorRampPalette(c(x,'white'))(5)[4]) %>% unlist()

p.geo.active$gamma_anomaly <- as.numeric(p.geo.active$gamma_anomaly)

g <- ggplot(p.geo.active, aes(x=gamma_anomaly,y=gamma_anomaly_p))
g2 <- g + geom_bar(aes(fill = active),stat = "identity",width=0.9)+
  scale_fill_manual(values = mycolor) +
  scale_x_continuous(
    breaks = c(1:7-0.5),  # 指定要显示的刻度
    labels = c(0,50,75,90,95,99,100)
  ) +
  #scale_y_continuous(breaks = c(0,146395*0.25,146395/2,146395*0.75,146395),labels = c(0,25,50,75,100)) + 
  xlab("Gamma_GI") + ylab("Proportion of cells (%)")+  labs(fill = 'Catagories') +
  theme_bw()+
  theme(legend.position = "none",
        ...
        legend.key.height = unit(0.5, "line"),
        legend.key.size = unit(0.5, "line"))
g2 <- g2+xlab('Quantiles')
ggsave('outputs/fig4/fig4_2.pdf',width = 1.6,height = 2.1,dpi='print')
# g2 + geom_errorbar(aes(x=gamma_anomaly,ymin=null.low,ymax = null.high),
#               color='black',width = 0.3)
# ggsave('outputs/fig4/fig4_2.pdf',width = 2,height = 2.5,dpi='print')
...
#pdat.s <- pdat.s[!pdat.s$active%in%'Inactive',]
#pdat.s$obs <- 100-pdat.s$obs[!pdat.s$active%in%'Inactive']
#pdat.s$null.mean <- 100-pdat.s$obs[!pdat.s$active%in%'Inactive']
...
pdat.s.2 <- pdat.s
pdat.s.2$active_obs <- paste(pdat.s.2$grp,pdat.s.2$active)
pdat.s.2$active <- factor(pdat.s.2$active,levels = c(
  "Active","Geomorphic active","Compression","Compressive shear",
  "Extensional shear","Extension"))
pdat.s.2$active.num <- as.numeric(pdat.s.2$active)

mycolor.col <- c(rev(mycolor),rev(mycolor))[c(1,7,2,8,3,9,4,10,5,11)]
mycolor.col <- c('black','gray',mycolor.col)
mycolor.fill <- mycolor.col
mycolor.fill[seq(2,12,2)] <- 'transparent'

ggplot(pdat.s.2, aes(x = active.num, y = obs,color=active)) +
  geom_bar(stat = "summary",
           fun ="mean",
           position = position_dodge(),
           alpha=0.7)+
  #scale_fill_manual(values = mycolor) +
  scale_color_manual(values = mycolor.col) +
  geom_errorbar(aes(x=active.num+0.25,ymin=null.low,ymax = null.high),
                color='gray30',width = 0.3)+
  facet_grid(.~gmm)+ theme_classic2()+
  ylab('')+
  theme(legend.position = "none",
        axis.ticks.x = element_blank(),
        axis.text.x = element_blank(),
        axis.title.x = element_blank())
ggsave('outputs/fig4/fig4_3.pdf',width = 7,height = 2.5,dpi='print')

# Fig 4-3 style  ----------------------------------------------------------

pdat.s.2 <- pdat.s
pdat.s.2$grp <- 'NULL'
pdat.s.2$obs <- pdat.s.2$null.mean
pdat.s.2 <- rbind(pdat.s,pdat.s.2)
pdat.s.2$active_obs <- paste(pdat.s.2$grp,pdat.s.2$active)

pdat.s.2$active_obs <- factor(pdat.s.2$active_obs,
                              levels = c("OBS Active","NULL Active",
                                         "OBS Geomorphic active","NULL Geomorphic active",
                                         "OBS Compression", "NULL Compression",
                                         "OBS Compressive shear","NULL Compressive shear",
                                         "OBS Extensional shear","NULL Extensional shear",
                                         "OBS Extension","NULL Extension"))
pdat.s.2$active <- factor(pdat.s.2$active,levels = c(
  "Active","Geomorphic active","Compression","Compressive shear",
  "Extensional shear","Extension"))
pdat.s.2$active.num <- as.numeric(pdat.s.2$active)
...
pdat.s.2.ori <- pdat.s.2

pdat.s.2 <- pdat.s.2.ori
pdat.s.2$active.num2 <- 1
pdat.s.2$null <- 'Observed'
pdat.s.2$null[str_detect(pdat.s.2$active_obs,'NULL')] <- 'Null model'
pdat.s.2$null <- factor(pdat.s.2$null,level=c('Observed','Null model'))
levels(pdat.s.2$gmm) <- c('Min~50%','50%~75%','75%~90%','90%~95%','95%~99%','99%~Max')

ggplot(pdat.s.2[!pdat.s.2$active%in%'Active',], aes(x = active.num2, y = obs,
                     fill = null,color=null)) +
                   geom_bar(stat = "summary",
                            fun ="mean",
                            position = position_dodge(),
                            alpha=0.7)+
  scale_fill_manual(values = mycolor.fill) +
  scale_color_manual(values = mycolor.col) +
  geom_errorbar(aes(x=active.num2+0.25,ymin=null.low,ymax = null.high),
                color='gray30',width = 0.3)+
  facet_grid(active~gmm)+ theme_classic(base_size = 8)+
  ylab('')+
  theme(#legend.position = c(0.1,0.8),
        ...
        axis.text.x = element_blank(),
        axis.title.x = element_blank())
ggsave('outputs/sup/categores/subcategores.jpg',width = 6,height = 6,dpi='print')

# p<- ggplot(pdat, aes(x = gamma_anomaly, 
#                      y = gamma_anomaly_p, 
#                      fill = obs)) +  
#   geom_bar(stat = "summary", 
#            fun ="mean", 
